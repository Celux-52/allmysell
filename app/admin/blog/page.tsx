...
export default function AdminBlogPage() {
  ...
  const statusColors: Record<string, string> = {
    ...
    DRAFT: 'bg-gray-500/10 text-stone-500',
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-stone-500 text-sm">{posts.length} total posts</p>
        </div>
        <Link
          href="/admin/blog/new"
          className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-purple-600 to-purple-500 text-stone-900 rounded-lg hover:shadow-lg hover:shadow-purple-500/20 transition-all font-medium text-sm"
        >
          <Plus size={18} />
          New Post
        </Link>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-stone-50 border border-white/10 rounded-lg text-stone-900 placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500/30 text-sm"
          />
        </div>
        <select
          value={filterStatus}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-stone-50 rounded-xl border border-white/5 p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredPosts.map((post) => (
            <div key={post.id} className="bg-stone-50 rounded-xl border border-white/5 hover:border-purple-500/20 transition-all p-5 group">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Link
                    ...
                    className="p-1.5 rounded-lg hover:bg-white/5 text-stone-500 hover:text-purple-400 transition-colors"
                  >
                    <Pencil size={14} />
                  </Link>
                  <button
              ...
              <h3 className="text-lg font-semibold text-stone-900 mb-2 line-clamp-2">{post.title}</h3>
