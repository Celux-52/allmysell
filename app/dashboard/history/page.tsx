...
export default function HistoryPage() {
  ...
  const formatRelativeTime = (isoString: string) => {
    ...
    if (diffInSeconds < 60) return t('hist.justNow');
    if (diffInSeconds < 3600) return `${Math.floor(diffInSeconds / 60)} ${t('hist.minAgo')}`;
    if (diffInSeconds < 86400) return `${Math.floor(diffInSeconds / 3600)}${t('hist.hAgo')}`;
    if (diffInSeconds < 172800) return t('hist.yesterday');
    ...
    <div className="space-y-8 pb-10 max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-bold text-white">{t('hist.title')}</h1>
          <p className="text-slate-400 mt-1">{t('hist.subtitle')}</p>
        </div>
      ...
      {activeTab === 'research' && (
        ...
        >
          ...
          <h2 className="text-xl font-bold text-white mb-2">{t('hist.noResearch')}</h2>
          <p className="text-slate-400 mb-6 max-w-sm mx-auto">{t('hist.noResearchDesc')}</p>
                  ...
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-3 text-xs text-slate-500 flex-wrap">
                        ...
                        <span>•</span>
                        <span className="text-slate-400">{item.resultCount} {t('hist.products')}</span>
                        {(item.topScore > 0) && (
                          <>
                            <span>•</span>
                            <span className="text-amber-400 font-medium">{t('hist.topScore')}: {item.topScore}</span>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    {trend && (
                      <div className={`flex items-center gap-1 text-xs px-2.5 py-1 rounded-full border ${
                        ...
                        {trend === 'up' && <><TrendingUp className="h-3 w-3" /> {t('hist.better')}</>}
                        {trend === 'down' && <><TrendingDown className="h-3 w-3" /> {t('hist.lower')}</>}
                        {trend === 'same' && <><Minus className="h-3 w-3" /> {t('hist.same')}</>}
                      </div>
                    )}
                    <button 
      ...
      {activeTab === 'etsy' && (
        etsyHistory.length === 0 ? (
          ...
          >
            ...
            <h2 className="text-xl font-bold text-white mb-2">{t('hist.noEtsy')}</h2>
            <p className="text-slate-400 mb-6 max-w-sm mx-auto">{t('hist.noEtsyDesc')}</p>
