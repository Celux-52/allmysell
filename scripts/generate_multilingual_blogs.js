...
function run() {
  ...
  if (!fs.existsSync(uzDir)) fs.mkdirSync(uzDir, { recursive: true });
  ...
  for (const f of files) {
    ...
    const trFile = fs.existsSync(path.join(trDir, f)) ? fs.readFileSync(path.join(trDir, f), 'utf-8') : enFile;

    if (BLOG_DATA[slug]) {
      const b = BLOG_DATA[slug];
      const ruContent = `---
title: "${b.ru.title}"
excerpt: "${b.ru.excerpt}"
date: "${b.ru.date}"
category: "${b.category}"
author: "Melih Bıçak"
dateISO: "2026-06-24"
...
${b.ru.content}`;
      ...
      const uzContent = `---
title: "${b.uz.title}"
excerpt: "${b.uz.excerpt}"
date: "${b.uz.date}"
category: "${b.category_uz}"
author: "Melih Bıçak"
dateISO: "2026-06-24"
---
${b.uz.content}`;
      ...
      const dateISO = meta.dateISO || '2026-06-24';
...
${body}`;
  ...
  console.log(`Successfully generated ${files.length} RU and UZ blog files.`);
}
